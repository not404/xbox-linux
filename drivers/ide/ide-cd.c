...
#ifdef CONFIG_X86_XBOX
//LALEE: This conditional doesn't smell right - logic below is written
// for the case where the DVD drive is dropped into a normal Linux-PC,
// which wouldn't have CONFIG_X86_XBOX to activate these quirks.
 	...
 	/* THOMSON DVD drives in the Xbox report incorrect capabilities
	   ...
	   can do the eject. */
    { "THOMSON-DVD", NULL, 
		IDE_CD_FLAG_XBOX_DRIVE | IDE_CD_FLAG_XBOX_EJECT | 
...
#ifdef CONFIG_X86_XBOX
//LALEE: This conditional doesn't smell right - logic here includes the case 
// where the DVD drive is dropped into a normal Linux-PC,
// which wouldn't have CONFIG_X86_XBOX to activate these quirks.

	/* Is an Xbox drive detected? */
	if (cd->cd_flags & IDE_CD_FLAG_XBOX_DRIVE) {
		...
		if (!machine_is_xbox) {
			if (cd->cd_flags & IDE_CD_FLAG_XBOX_EJECT) {
				cd->cd_flags |= (IDE_CD_FLAG_NO_DOORLOCK | IDE_CD_FLAG_NO_EJECT);
			}
		} else {
